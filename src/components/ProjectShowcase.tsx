...
const ProjectShowcase = () => {
  ...
  const projects: Project[] = [{
    ...
    metrics: [{
      title: 'Truck Reduction',
      value: '40%',
      description: 'Fewer trucks needed through optimization'
    }, {
      title: 'Water Efficiency',
      value: '60%',
      description: 'Improved consumption planning'
    }, {
      title: 'Cost Offset',
      value: '100%',
      description: 'Project costs fully recovered'
    }],
    demoAnimation: {
      type: 'chart',
      data: {}
    ...
    metrics: [{
      ...
      value: '60%',
      description: 'Less unexpected breakdowns'
    }, {
      title: 'Maintenance Costs',
      value: '-25%',
      description: 'Reduction in repair expenses'
    }, {
      title: 'Fleet Efficiency',
      value: '+40%',
      description: 'Improved vehicle utilization'
    }],
    demoAnimation: {
      type: 'flow',
      data: {}
    ...
    metrics: [{
      ...
      value: '75%',
      description: 'Faster model to production'
    }, {
      title: 'Accuracy Improvement',
      value: '+20%',
      description: 'Better model performance'
    }, {
      title: 'Developer Productivity',
      value: '3x',
      description: 'More models deployed per sprint'
    }],
    demoAnimation: {
      type: 'metrics',
      data: {}
  ...
  const SkeletonCard = () => <Card className="relative h-80 overflow-hidden border-2 border-border/30">
  ...
  return <section className="py-20 bg-gradient-to-b from-background via-background/95 to-accent/5 relative overflow-hidden" aria-label="Project showcase section">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl lg:text-5xl font-bold text-foreground mb-6 bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text">What We've Built</h2>
        ...
        {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            {projects.map((project, index) => <div key={project.id} className="group transition-all duration-700 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] will-change-transform hover:scale-[1.03] hover:-translate-y-2" style={{
            ...
            handleCardClick(project.id);
          }} onMouseEnter={() => handleCardHover(project.id, true)} onMouseLeave={() => handleCardHover(project.id, false)} onFocus={() => setFocusedCard(project.id)} onBlur={() => setFocusedCard(null)} onKeyDown={e => handleKeyPress(e, project.id)} tabIndex={0} role="button" aria-expanded={expandedCard === project.id} aria-label={`${project.client} project: ${project.title}. Click to expand details.`}>
                  {/* Card Flip Container */}
              ...
              transformStyle: 'preserve-3d'
                    ...
                    <div className={`
                        ...
                        ${flippedCards.has(project.id) ? 'opacity-0 pointer-events-none' : 'opacity-100'}
                      `} style={{
                ...
                backfaceVisibility: 'hidden'
                      ...
                      <div className="space-y-6">
                        <div className="flex flex-col items-center space-y-4">
                          <div className="relative group-hover:scale-110 transition-transform duration-300">
                            <img src={project.logo} alt={`${project.client} logo`} className="w-12 h-12 object-contain drop-shadow-lg transition-all duration-300" onError={e => {
                    ...
                    <div className={`
                        ...
                        transition-all duration-700 ease-[cubic-bezier(0.175,0.885,0.32,1.275)]
                        will-change-transform
                        ${flippedCards.has(project.id) ? 'opacity-100' : 'opacity-0 pointer-events-none'}
                      `} style={{
                ...
                backfaceVisibility: 'hidden'
                      ...
                      <div className="h-full flex flex-col justify-center items-center text-center space-y-4 overflow-hidden">
                        <div className="space-y-4 w-full px-2">
                          {project.metrics.slice(0, 2).map((metric, idx) => <div key={idx} className="transform hover:scale-105 transition-transform duration-300" style={{
                      animationDelay: `${idx * 100}ms`
                    }}>
                              <div className="text-4xl lg:text-5xl font-extrabold text-primary mb-1 bg-gradient-to-r from-primary to-primary/80 bg-clip-text animate-fade-in-up" style={{
      ...
      <Dialog open={!!expandedCard} onOpenChange={handleCloseExpanded}>
        <DialogContent className="max-w-7xl w-[95vw] max-h-[95vh] p-0 overflow-hidden bg-card/95 backdrop-blur-md border border-primary/20">
          ...
          return <div className="h-full max-h-[95vh] overflow-y-auto">
                    ...
                    <div className="flex flex-col space-y-6">
                      <div className="flex items-center space-x-4">
                        <div className="relative">
                          <img src={project.logo} alt={`${project.client} logo`} className="w-16 h-16 object-contain drop-shadow-lg" />
                      ...
                      <div className="space-y-4">
                        ...
                        {project.metrics.map((metric, idx) => <div key={idx} className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-accent/20 to-accent/10 border border-accent/30">
                            <div>
                      ...
                      <h5 className="text-2xl font-bold text-foreground text-center">
                       ...
                       <div className="w-full bg-gradient-to-br from-primary/10 via-primary/5 to-primary/10 rounded-xl border-2 border-primary/20 overflow-hidden">
                         <div className="relative w-full aspect-video bg-gradient-to-br from-muted/50 to-muted/30 flex items-center justify-center">
                           {/* Background Image */}
                           <img src={project.videoFull} alt={`${project.client} solution demo`} className="absolute inset-0 w-full h-full object-cover opacity-80" />
                           ...
                           {/* AWS Water Infrastructure Animated Demo */}
                            {project.id === '1' && playingVideo === project.id && <>
                                ...
                                <div className="absolute inset-0 overflow-hidden pointer-events-none">
                                  ...
                                  <div className="absolute w-full h-1 bg-gradient-to-r from-blue-400/80 via-blue-500/60 to-blue-600/40 top-1/3 animate-pulse" style={{
                            animationDelay: '0.5s'
                          }}></div>
                                  <div className="absolute w-1 h-full bg-gradient-to-b from-blue-400/80 via-blue-500/60 to-blue-600/40 right-1/4 animate-pulse" style={{
                            animationDelay: '1s'
                          }}></div>
                                  
                                  {/* Water Flow Particles */}
                                  {[...Array(8)].map((_, i) => <div key={`particle-${i}`} className="absolute w-2 h-2 bg-cyan-300 rounded-full" style={{
                            left: i % 2 === 0 ? '25%' : '75%',
                            ...
                            top: `${10 + i * 10}%`
                                  ...
                                  <div className="absolute bottom-8 left-8 w-16 h-20 bg-gray-700/60 backdrop-blur-sm rounded-t-lg border border-gray-500/40">
                                    ...
                                    <div className="w-full h-1 bg-blue-400/60 animate-pulse mt-1" style={{
                              animationDelay: '0.3s'
                            }}></div>
                                  ...
                                  <div className="absolute bottom-8 right-8 w-16 h-20 bg-gray-700/60 backdrop-blur-sm rounded-t-lg border border-gray-500/40">
                                    <div className="w-full h-2 bg-green-400/80 animate-pulse mt-2" style={{
                              animationDelay: '0.8s'
                            }}></div>
                                    <div className="w-full h-1 bg-blue-400/60 animate-pulse mt-1" style={{
                              animationDelay: '1.1s'
                                  ...
                                  <div className="absolute top-8 left-8 w-32 h-20 bg-black/40 backdrop-blur-sm rounded-lg border border-green-400/40 p-2">
                                    ...
                                    <div className="flex items-end space-x-1 h-12">
                                      {[...Array(8)].map((_, i) => <div key={`bar-${i}`} className="bg-gradient-to-t from-green-400 to-green-300 w-2 rounded-t" style={{
                                ...
                                animationDelay: `${i * 0.2}s`
                                  ...
                                  animationDelay: '0.5s'
                                      ...
                                      <div className="flex items-center space-x-2">
                                        <div className="w-2 h-2 bg-green-400 rounded-full animate-bounce" style={{
                                  animationDelay: '1s'
                                }}></div>
                                ...
                                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-black/60 backdrop-blur-sm px-6 py-4 rounded-xl border border-yellow-400/40">
                                  <div className="text-center">
                                    ...
                                    <div className="w-32 h-2 bg-gray-600 rounded-full overflow-hidden">
                                      <div className="h-full bg-gradient-to-r from-yellow-400 to-green-400 rounded-full transition-all duration-2000 ease-out" style={{
                                width: '96.2%'
                              }}></div>
                                    </div>
                                  </div>
                           ...
                           <div className="absolute inset-0 bg-gradient-to-br from-primary/20 to-primary/5 animate-pulse"></div>
                            ...
                            <div className="relative z-10 text-center">
                              <button className="w-20 h-20 bg-primary/30 rounded-full flex items-center justify-center shadow-2xl backdrop-blur-sm border-2 border-primary/40 hover:bg-primary/40 active:bg-primary/50 transition-all duration-300 group mb-4" aria-label="Play solution demo" onClick={e => {
                          e.stopPropagation();
                          handlePlayVideo(project.id);
                        }}>
                                {playingVideo === project.id ? <div className="w-4 h-4 bg-white rounded-sm animate-pulse"></div> : <div className="w-0 h-0 border-l-[16px] border-l-white border-y-[12px] border-y-transparent ml-1 group-hover:scale-110 group-active:scale-95 transition-transform duration-200"></div>}
                              </button>
                           ...
                           {project.id !== '1' && <div className="absolute inset-0 overflow-hidden pointer-events-none">
                               {[...Array(3)].map((_, i) => <div key={i} className="absolute w-1 h-8 bg-gradient-to-b from-primary/40 to-transparent rounded-full" style={{
                          left: `${20 + i * 30}%`,
                          top: '10%',
